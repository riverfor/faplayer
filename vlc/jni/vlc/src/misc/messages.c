 ...
 * $Id: 8bc8b16e451ca49460231e008167b04aecf23ea8 $
 *
                      ...
                      const char *format, va_list ap)
{
    ...
    funlockfile (stream);
#ifdef __ANDROID__
    char *sz_prefix, *sz_message;
    int ret = asprintf(
        &sz_prefix,
        p_item->psz_header ? "[%p][%s] %s %s" : "[%p]%s%s %s",
    ...
    if (ret < 0)
        goto no_mem2;
    ret = vasprintf(&sz_message, format, ap);
    if (ret < 0)
        goto no_mem1;
    __android_log_print(ANDROID_LOG_DEBUG, "faplayer", "%s: %s", sz_prefix, sz_message);
    free(sz_message);
no_mem1:
    free(sz_prefix);
no_mem2:
